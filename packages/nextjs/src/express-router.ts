...
function registerController(
  ...
  if (!controllerMeta) {
    console.warn(`[AI-First] No @RestController metadata found on ${ControllerClass.name}`);
    return;
  }
  ...
  for (const [methodName, mapping] of Object.entries(mappings)) {
    ...
    const handler: Handler = async (req, res, _next) => {
      ...
      try {
        ...
        if (verbose) {
          console.log(`[AI-First] ← ${httpMethod.toUpperCase()} ${fullPath} 200 (${Date.now() - start}ms)`);
        }

        res.json({ success: true, data: result });
      } catch (error: any) {
        if (verbose) {
          console.error(`[AI-First] ← ${httpMethod.toUpperCase()} ${fullPath} 400 (${Date.now() - start}ms) ${error.message}`);
        }
